...
export async function generateMetadata({ params }: any) {
    ...
    const canonicalUrl = `${baseUrl}/haqqimizda`;
    if (data.error) {
        return {
            alternates: {
        ...
        <div className="wrapper w-full pt-30">
            ...
            <div className='base:mt-30'>
                {about.sections.map((item: any, index: number) => (
                    <div key={index} className={`md:mt-30 grid md:grid-cols-2 justify-between gap-7 base:gap-33 ${index % 2 ? 'md:flex-row-reverse' : 'md:flex-row'}`}>
                        <div className={`flex-1/2 rounded-[20px] ${ index % 2 && '-order-1' }`}>
                            {item?.left_side?.type === 'image' ? (
                                <img  src={item?.left_side?.url} alt="Handex-missiyamiz" className='object-cover md:block hidden pt-10 h-100 w-full rounded-[20px]' /> 
                            ) : (
                        ...
                        <div className={`flex-1/2 rounded-[20px]`}>
                            {item?.right_side?.type === 'image' ? (
                                <img src={item?.right_side?.url} alt="Handex-haqqimizda" className='object-cover md:block hidden pt-10 h-100 w-full rounded-[20px]' />
                            ) : (
