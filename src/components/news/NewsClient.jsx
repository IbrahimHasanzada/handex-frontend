    ...
    <div className='wrapper pt-25 overflow-x-hidden'>
      <div className='md:py-13 md:px-9 md:bg-white mb-30 md:mt-20 mt-6 rounded-2xl md:flex items-center justify-between'>
        ...
        <div className='lg:w-2/5 md:w-1/2 w-full h-12 px-4 border border-[#909090] rounded-[20px] md:rounded-[30px] relative'>
          <input onChange={(e) => handleChange(e.target.value)} type="text" placeholder="Xəbəri axtar..." className='text-[#909090] outline-none h-full w-full rounded-[20px]' />
      ...
      {total > (count + 1) * 12 && (
        <button id='paginate-button' onClick={() => handlePagination()} className='flex bg-handle-gray mx-auto rounded-full items-center px-6 gap-2 h-12 my-15'>
