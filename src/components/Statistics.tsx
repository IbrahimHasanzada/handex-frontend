                    ...
                    {t.raw("home.statistics.statisticsValue").map((item: any, i: number) => {
                        return (
                            <div key={i} className={`${page !== 'corporate' && 'max-w-54'} text-center`}>
                                <p className={`${page === 'corporate' ? 'bg-gradient-to-r from-[#F4F5F5] to-[#666] text-transparent bg-clip-text' : 'text-[#141414]'} font-bold text-[5rem] `}>{(i % 2 ? '+' : '') + Object.values(data)[i]}</p>
